...
// Member-facing page. Shows their personal invite code, share link, QR code,
// counts of pending/approved referrals, and lets them regenerate or revoke.
// ============================================================================
...
export default async function InvitesPage() {
  ...
  if (!session?.user?.id) {
    redirect("/auth/signin?next=/invites");
  }
  ...
  // Make sure they have a code (generates one if not)
  await ensureMyCode();
  ...
  // Referral counts (across all of this member's codes — past and present)
  const [{ pendingCount }] = await db
    .select({ pendingCount: sql<number>`count(*)::int` })
    .from(applications)
    .where(
      and(
        eq(applications.invitedBy, me.id),
  ...
  const appUrl =
    process.env.NEXT_PUBLIC_APP_URL ?? "https://undergroundgallery.ai";
  ...
  return (
    ...
    >
      <div style={{ maxWidth: 900, margin: "0 auto", padding: "40px 24px" }}>
        ...
        >
          Anyone who applies with your code is tied to you forever. Build the
        ...
        {/* SHARE CARD (client-side QR + copy buttons) */}
            ...
            <details>
              ...
              >
                <p style={{ fontSize: 12, color: "#aaa", marginBottom: 12 }}>
                  Regenerating creates a fresh code and revokes the old one.
                  People with the old code or QR will get an error.
                </p>
                <form action={regenerateMyCode}>
                  <button
                    type="submit"
...
function Stat({
  ...
  return (
    <div
      style={{
        background: highlight && value > 0 ? "rgba(255,48,48,0.08)" : colors.surface,
